...
class Funcs():
    ...
    def projectIn2D(self,pdFrame, dimred):
        ...
        try:
            ...
            fig = px.scatter(df_plot,  x="C1", y="C2", color="Classi") 
    ...
    def makeCrosval(self,URL, pdFrame,model,dimred,X_data):
        ...
        print(X_data,y_pred_data)
        scores = cross_val_score(estimator=pipe,X=X_train,y=y_train,cv=10,n_jobs=1)
        singleScore, crossValScore, meanScore = pipe.score(X_test, y_test),scores,(round(np.mean(scores*100),2), round(np.std(scores*100),0))

        return y_pred,y_pred_data,singleScore, crossValScore, meanScore


    def modelScoringCrossVal(self, URL,pdFrame, dimred, X_data):
        ...
        try:
            ...
            for i,model in enumerate(models):
                y_pred,y_pred_data,singleScore, crossValScore, meanScore=self.makeCrosval(self,URL, pdFrame,model,dimred,X_data)
                scores.append(meanScore)
                predicted.append(y_pred_data)
            CV.cv_scores=scores
            CV.save()

        return scores,modelNames, predicted

    def optimizeHyperParameters(self, pdFrame,URL):
        try:
            OHP=OptimizedHyperParameters.objects.get(url=URL)
            optimizedScores,modelNames, bestSingle, bestParameters=OHP.optimizedScores,OHP.modelNames, OHP.bestSingle,OHP.bestParameters
            ...
            for i, model in enumerate(models):
                ...
                try:
                    ...
                    bestParametersSet=gs.fit(X_train, y_train).best_params_
                except:

                    meanScore=(round(np.mean(scores)*100,2), round(np.std(scores)*100,0))
                    optimizedScores.append(meanScore)
                    bestSingle[modelNames[i]]=[bestSingelScore]
                    bestParameters.append(bestParametersSet)
            table=BaseData.objects.get(url=URL)
            OHP=OptimizedHyperParameters()
            OHP.optimizedScores,OHP.modelNames, OHP.bestSingle, OHP.bestParameters=optimizedScores,modelNames, bestSingle,bestParameters
            OHP.baseData=table
            OHP.url=URL
            OHP.save()
            
        return optimizedScores,modelNames, bestSingle,bestParameters

    def majorityVote(self, pdFrame, bestParameters, modelNames,URL):
        try:
            ...
            #####################################
            #################################
            max_depth_DT=bestParameters[0]['max_depth']
            C_svm,kernel_svm=bestParameters[1]['svc__C'],bestParameters[1]['svc__kernel']
            metric_knn, n_neighbors_knn=bestParameters[2]['metric'], bestParameters[2]['n_neighbors']
            C_lr, solver_lr=bestParameters[3]['C'], bestParameters[3]['solver']
            max_depth_RF, n_estimators_RF=bestParameters[4]['max_depth'], bestParameters[4]['n_estimators']
